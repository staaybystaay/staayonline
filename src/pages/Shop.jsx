...
// ─── PRODUCT CARD — GRID ─────────────────────
function ProductCardGrid({ product, wishlisted, onWishlist }) {
...
// ─── PRODUCT CARD — LIST ─────────────────────
function ProductCardList({ product, wishlisted, onWishlist }) {
  const [hovered, setHovered] = useState(false)
  const addItem = useCartStore(s => s.addItem)
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {loading ? (
            ...
            <motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '32px 20px' }}>
              {sorted.map(p => (
                <ProductCardGrid key={p.id} product={p} wishlisted={wishlist.includes(p.id)} onWishlist={toggleWishlist} />
              ))}
            </motion.div>
          ) : (
            <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {sorted.map(p => (
                <ProductCardList key={p.id} product={p} wishlisted={wishlist.includes(p.id)} onWishlist={toggleWishlist} />
              ))}
            </motion.div>
          )}
        </div>
